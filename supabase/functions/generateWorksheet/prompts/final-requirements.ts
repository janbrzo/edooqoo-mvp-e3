...
export const getFinalRequirements = (hasGrammarFocus: boolean, exerciseCount: number = 8, selectedExercises?: string[], englishLevel?: string, hasSelectedImage?: boolean, hasSelectedAudio?: boolean) => {
  ...
  const exerciseRequirements = {
    ...
    'odd-one-out': `EXACTLY 10 groups with EXACTLY 5 options each. CRITICAL: In each group, 4 words MUST be of the SAME grammatical category (same part of speech: noun, verb, adjective, adverb, gerund, etc.), and 1 word MUST be of a DIFFERENT grammatical category. This grammatical difference is the basis for identifying the odd one out. Examples: 4 verbs + 1 noun ("eat, drink, sleep, food, run" → odd: "food"), 4 adjectives + 1 verb ("happy, sad, beautiful, run, cold" → odd: "run"), 4 nouns + 1 adjective ("table, chair, book, red, pen" → odd: "red"). IMPORTANT: Randomize the position of the odd word within the options array - it should NOT consistently appear in the last or second-to-last position; vary the position across different groups. Use only single vocabulary words. Exclude proper names, city names, and sentences. Ensure all content matches CEFR level ${englishLevel || 'as specified'}.`,
    ...
    'fill-in-blanks-audio': `EXACTLY 10 sentences based on the audio transcript with one blank (______) per sentence. Use "sentences" array (each with "text" and "answer" fields) and "word_bank" array containing all 10 answers. Each sentence must have ONE blank represented by underscores. Word bank should list all answers in mixed order. Focus on key vocabulary and challenging words from the audio. Ensure all content matches CEFR level ${englishLevel || 'as specified'}.`,
