-- Fix consume_token function - Remove FREE DEMO WEEK and fix SQL operator error
-- This fixes Problem 1: Token consumption failure
...
BEGIN
  ...
  IF current_available > 0 THEN
    -- Determine if this should count towards monthly usage
    should_count_monthly := (COALESCE(sub_status, '') IN ('active','active_cancelled'))
                            OR (COALESCE(sub_type, '') <> 'Free Demo')
                            OR (total_created >= 2);

    -- Decrement available tokens AND increment consumed
    UPDATE public.profiles 
    SET 
