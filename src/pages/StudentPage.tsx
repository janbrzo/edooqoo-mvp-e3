...
function MeetingLinkField({ studentId, teacherId, hasGcal }: { studentId: string; teacherId: string; hasGcal?: boolean }) {
  ...
  useEffect(() => {
    const load = async () => {
      // Check if auto-create permanent links is enabled
      ...
      const autoEnabled = !!(calSettings as any)?.auto_create_student_meeting_link;
      setAutoLinkEnabled(autoEnabled);

      // Generate the deterministic fallback link
      const { generateFallbackMeetingLink, isAutoGeneratedLink } = await import('@/utils/meetingLinkUtils');
      const fallback = generateFallbackMeetingLink(teacherId, studentId);
      setGeneratedLink(fallback);

      // Fetch current saved link
      ...
      const savedLink = data?.default_meeting_link || '';
      setLink(savedLink);

      if (autoEnabled) {
        // Determine mode: if saved link is auto-generated or empty → default, otherwise → custom
        if (!savedLink || isAutoGeneratedLink(savedLink)) {
          setMode('default');
        } else {
          setMode('custom');
        }
  ...
  const handleSave = async (linkToSave: string | null) => {
    ...
    if (existing) {
      await supabase.from('calendar_student_settings').update({ default_meeting_link: linkToSave, updated_at: new Date().toISOString() } as any).eq('id', existing.id);
    } else {
      await supabase.from('calendar_student_settings').insert({ student_id: studentId, teacher_id: teacherId, default_meeting_link: linkToSave } as any);
    }
    ...
    if (linkToSave) {
      try {
        ...
        await supabase.from('calendar_slots')
          .update({ meeting_link: linkToSave } as any)
          .eq('teacher_id', teacherId)
          .eq('student_id', studentId)
          .gte('slot_date', today)
  ...
  const handleModeChange = async (newMode: 'default' | 'custom') => {
    ...
    if (newMode === 'default') {
      ...
      await handleSave(generatedLink);
    }
  };
    ...
    <div>
        ...
        <Input
          ...
          onBlur={() => {
            if (mode === 'custom' || !autoLinkEnabled) {
              handleSave(link || null);
            }
          }}
          placeholder="https://meet.google.com/..."
          className="h-8 text-sm"
          disabled={autoLinkEnabled && mode === 'default'}
        />
        {currentLink && (
          <button
      ...
      <p className="text-xs text-muted-foreground mt-1">
        {currentLink
          ...
          : hasGcal
            ? "Connect Google Meet in Calendar Settings or paste your meeting room link (e.g., Google Meet, Zoom). Students will see a 'Join Lesson' button."
            : "Connect Google Meet in Calendar Settings or paste your meeting room link (e.g., Google Meet, Zoom). Students will see a 'Join Lesson' button."
        }
      </p>
    </div>
